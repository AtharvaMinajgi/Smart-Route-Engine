...
public class BanditService {
     ...
     * 70% ML probability
     * 30% Thompson Sampling exploration
     */
    public Route selectRouteWithML(double amount, MLClientService mlClientService) {
        ...
        for (Route route : routes) {
            ...
            //️⃣ Thompson Sampling (exploration)
            double banditSample = sampleBeta(stats.getAlpha(), stats.getBeta());

            // Estimate latency from rolling metrics
            int estimatedLatency = routeMetricsRepository
                    .findByRoute(route)
                    .map(m -> m.getAvgLatencyMs().intValue())
                    .orElse(1000);

            //️Ask ML model for probability
            Double mlProbability = mlClientService
                    .predictSuccess(route.getName(), amount, estimatedLatency);
